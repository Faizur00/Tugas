...
### Descriptive Text
Algoritma: Pencarian nomor telpon berdasarkan alamat dengan pelayanan lokal

1. Mulai program.
2. Tetapkan nomor layanan ke 108.
3. Pengguna menghubungi nomor layanan.
4. Jika nomor layanan tidak sibuk:
    a. Pengguna memasukkan nama dan nomor alamat.
    b. Layanan mencari nomor sesuai masukan pengguna.
    c. Jika alamat memiliki nomor telepon:
        i. Layanan memberikan nomor kepada pengguna.
    d. Jika tidak:
        i. Layanan memberitahu bahwa alamat tidak memiliki nomor.
5. Jika nomor layanan sibuk:
    a. Pemberitahuan sibuk muncul.
6. Sambungan diputus.
7. Program selesai.
...
BEGIN
    NOMOR LAYANAN = 108
    PENGGUNA MENGHUBUNGI NOMOR LAYANAN

    IF (NOMOR LAYANAN != SIBUK) DO:
        PENGGUNA MEMASUKKAN NAMA DAN NOMOR ALAMAT
        LAYANAN MENCARI NOMOR SESUAI MASUKAN PENGGUNA
            IF (ALAMAT PUNYA NO TELPON) DO:
                LAYANAN MEMBERI NOMOR KEPADA PENGGUNA
            ELSE
                LAYANAN MEMBERITAHU BAHWA ALAMAT TIDAK MEMILIKI NOMOR
    ELSE
        PEMBERITAHUAN SIBUK MUNCUL
...
void cari_nomor_telepon(char alamat[]) \
{
    int nomor_108_sibuk = 0;
    int alamat_terdaftar = 1;

    if (nomor_108_sibuk) {
        printf("Nomor 108 sibuk. Coba lagi nanti.\n");
    } else {
        printf("Terhubung ke nomor 108 PT Telkom.\n");
        if (alamat_terdaftar) {
            printf("Nomor telepon untuk alamat %s adalah 08123456789.\n", alamat);
        } else {
            printf("Alamat %s belum memiliki sambungan telepon.\n", alamat);
        }
    }
}
...
{
    char alamat[100];
    printf("Masukkan alamat rumah (nama jalan dan nomor rumah): ");
